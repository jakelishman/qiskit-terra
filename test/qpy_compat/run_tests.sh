...
"$qiskit_venv/bin/pip" install -c "$repo_root/constraints.txt" "$repo_root" packaging
...
for i in "${!symengine_versions[@]}"; do
    ...
    "$symengine_venv/bin/pip" install -c "$repo_root/constraints.txt" "$repo_root" "symengine$specifier"
    mkdir -p "$files_dir"
    pushd "$files_dir"
